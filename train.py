...
exp_args.START_STEP = 4_000_000 # to restart from previous crash point
exp_args.NUM_STEPS = exp_args.START_STEP + 10_000_000 # number of steps/frames to train on
exp_args.EXPLORE_STEPS = 5_000_000 # eps will decay from 1 towards .1 lineary from step 0 -> EXPLORE_STEPS
exp_args.MODEL_NAME = 'hd_gru_8f'
exp_args.TARGET_UPDATE_FREQ = 1000 # frequency in steps/frames to update target model weights 
exp_args.STORE_DENSITY = 100 # store the model in the nearest STORE_DENSITY (episode number) indexed file, to avoid saving too many models
...
exp_args.EVAL = False # whether to execute evaluation to run testing regiment
